...
def get_frame(driver):
    ...
    actual_width = driver.execute_script(
        "return Runner.instance_.canvas.width")
    dino_width = driver.execute_script(
    ...
    try:
        ...
        frame = crop_and_resize(screenshot, canvas_details, actual_width, dino_width, xPos)
        return frame
...
for episode in range(EPISODES):
    ...
    while not done and step < MAX_STEPS:
        ...
        if np.random.rand() < epsilon:
            ...
            if step % 10 == 0:
                print(f"[EP {episode+1}] Step {step} | Epsilon: {epsilon:.4f} | Q-values: {q_values[0]}")
...
if len(reward_history) >= 10:
    plt.plot(moving_avg(reward_history),
             label="Moving Avg (10)", linestyle="--")
